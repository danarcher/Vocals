using System;
using System.Data;
using System.Linq;
using System.Speech.Recognition;
using System.Windows.Forms;
using Vocals.InternalClasses;

namespace Vocals
{
    public partial class FormOptions : Form
    {
        public Options options;

        public FormOptions()
        {
            InitializeComponent();
        }

        private void FormOptions_Load(object sender, EventArgs e)
        {
            Keys[] keyDataSource = (Keys[])Enum.GetValues(typeof(Keys)).Cast<Keys>();
            comboBox2.DataSource = keyDataSource;

            recognitionLanguageComboBox.DataSource = getInstalledRecognitionLanguages();

            options = new Options();

            checkBox1.Checked = options.toggleListening;
            comboBox2.SelectedItem = options.key;
            richTextBox1.Text = options.answer;
            trackBar1.Value = options.threshold;
            label5.Text = Convert.ToString(options.threshold);
            recognitionLanguageComboBox.SelectedItem = options.language;
            recognitionLanguageWarning.Visible = false;

            if (checkBox1.Checked)
            {
                comboBox2.Enabled = true;
                richTextBox1.Enabled = true;
            }
            else
            {
                comboBox2.Enabled = false;
                richTextBox1.Enabled = false;
            }
        }

        private string[] getInstalledRecognitionLanguages()
        {
            return SpeechRecognitionEngine.InstalledRecognizers().Select(ri => ri.Culture.DisplayName).ToArray();
        }

        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (options != null)
            {
                if (checkBox1.Checked)
                {
                    comboBox2.Enabled = true;
                    richTextBox1.Enabled = true;
                    options.toggleListening = true;
                }
                else
                {
                    comboBox2.Enabled = false;
                    richTextBox1.Enabled = false;
                    options.toggleListening = false;
                }
            }
        }

        private void trackBar1_Scroll(object sender, EventArgs e)
        {
            if (options != null)
            {
                options.threshold = trackBar1.Value;
                label5.Text = Convert.ToString(trackBar1.Value);
            }
        }

        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (options != null)
            {
                options.key = (Keys)comboBox2.SelectedItem;
            }
        }

        private void richTextBox1_TextChanged(object sender, EventArgs e)
        {
            if (options != null)
            {
                options.answer = richTextBox1.Text;
            }
        }

        private void recognitionLanguageComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (options != null)
            {
                options.language = (String)recognitionLanguageComboBox.SelectedItem;
                recognitionLanguageWarning.Visible = true;
            }
        }
    }
}
